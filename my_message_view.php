...
try {
  ...
  error_log('my_message_view error: ' . $e->getMessage());
  header('Location: /my_messages.php?err=' . urlencode($e->getMessage() ?: 'view_failed')); exit;
}
...
<div class="flex items-center justify-between mb-6">
  <div class="min-w-0">
    <div class="text-sm text-gray-500 mb-1">
      <a class="hover:underline" href="/my_messages.php"><?= __('messages.inbox') ?: '쪽지함' ?></a>
  ...
  <div class="shrink-0 flex gap-2">
    <!-- 보관/해제 토글은 목록 화면에서 주로, 필요하면 여기서도 제공 가능 -->
    <form method="post" action="/message_action.php" class="hidden">
      <input type="hidden" name="csrf" value="<?= htmlspecialchars(csrf_token()) ?>">
      <input type="hidden" name="action" value="archive">
      <input type="hidden" name="thread_id" value="<?= (int)$thread['id'] ?>">
      <button class="px-3 py-1.5 rounded border text-sm"><?= __('messages.archive') ?: '보관' ?></button>
    </form>
  </div>
</div>
      ...
      <?php foreach ($msgs as $m): ?>
        ...
        <div class="flex <?= $mine ? 'justify-end' : 'justify-start' ?>">
          <div class="max-w-[80%]">
            ...
            <?php if ($mine || (isset($me['role']) && strtolower($me['role']) === 'admin')): ?>
              <!-- 내 메시지 소프트 삭제 (선택) -->
...
<div class="mt-4 bg-white border rounded-lg p-4">
  <form method="post" action="/message_send.php" class="space-y-2">
    ...
    <input type="hidden" name="thread_id" value="<?= (int)$thread['id'] ?>">
    <!-- 현재 화면으로 돌아오기 -->
    <input type="hidden" name="return" value="/my_message_view.php?thread_id=<?= (int)$thread['id'] ?>">
    ...
    <div class="flex items-center justify-between mt-2">
      <a href="/my_messages.php" class="text-sm text-gray-600 hover:underline"><?= __('common.back') ?: '목록으로' ?></a>
