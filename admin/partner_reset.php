<?php
// 대기중 = partner_applications.status='pending', reviewer/reset
...
$pdo = db();
$st = $pdo->prepare("UPDATE partner_applications
  SET status='pending', reviewer_id=NULL, reviewed_at=NULL, updated_at=NOW()
  WHERE id=:id");
$st->execute([':id'=>$appId]);

$back = $_POST['return'] ?? '/admin/partners.php';
header('Location: ' . $back . (str_contains($back,'?')?'&':'?') . 'msg=reset');